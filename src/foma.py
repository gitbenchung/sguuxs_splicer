...
class FST(object):
    ...
    def define(cls, definition, name):
        ...
        if isinstance(definition, FST):
            retval = foma.add_defined(c_void_p(cls.networkdefinitions.defhandle), foma_fsm_copy(
                definition.fsthandle), c_char_p(name))
        elif isinstance(definition, FST.string_type):
            regex = cls.encode(definition)
            retval = foma.add_defined(c_void_p(cls.networkdefinitions.defhandle), foma_fsm_parse_regex(c_char_p(
                regex), c_void_p(cls.networkdefinitions.defhandle), c_void_p(cls.functiondefinitions.deffhandle)), c_char_p(name))
        else:
        ...
        if isinstance(definition, FST.string_type):
            ...
            for i in range(numargs):
                definition = definition.replace(
                    prototype[1][i], "@ARGUMENT0%i@" % (i+1))
            regex = cls.encode(definition + ';')
            retval = foma.add_defined_function(c_void_p(
                cls.functiondefinitions.deffhandle), c_char_p(name), c_char_p(regex), c_int(numargs))
        else:
    ...
    def load(cls, filename):
        ...
        fsm = cls()
        fsm.fsthandle = foma_fsm_read_binary_file(
            c_char_p(FST.encode(filename)))
    ...
    def __init__(self, regex=False):
        if regex:
            ...
            self.fsthandle = foma_fsm_parse_regex(c_char_p(self.regex), c_void_p(
                self.networkdefinitions.defhandle), c_void_p(self.functiondefinitions.deffhandle))
    ...
    def __getitem__(self, key):
        ...
        result = []
        output = foma_apply_down(
            c_void_p(self.getitemapplyer), c_char_p(self.encode(key)))
        while True:
            if output == None:
                return result
            else:
    ...
    def _apply(self, applyf, word=None, tokenize=False):
        ...
        if tokenize:
            toksym = '\x07'
            foma_apply_set_space_symbol(
                c_void_p(applyerhandle), c_char_p(toksym))
        if word:
            output = applyf(c_void_p(applyerhandle),
                            c_char_p(self.encode(word)))
        else:
            output = applyf(c_void_p(applyerhandle))
        while True:
            if output == None:
    ...
    def intersect(self, other, minimize=True):
        new = FST()
        new.fsthandle = self._fomacallbinary(
            other, foma_fsm_intersect, minimize)
        return new
    ...
    def flatten(self):
        new = FST()
        eps_sym = FST('□')
        new.fsthandle = foma_fsm_flatten(foma_fsm_copy(
            self.fsthandle), foma_fsm_copy(eps_sym.fsthandle))
        return new
...
class MTFSM(FST):
    ...
    def __init__(self, regex=False, numtapes=2):
        if isinstance(regex, FST.string_type):
            ...
            eps_sym = FST('□')
            self.fsthandle = foma_fsm_flatten(foma_fsm_copy(
                self.fsthandle), foma_fsm_copy(eps_sym.fsthandle))
    ...
    def join(self, other):
        ...
        m = self.numtapes
        n = other.numtapes
        pada = FST('[0:□^' + str(m) + ' [0:?^' + str(n-1) + ' - 0:□^' +
                   str(n-1) + '] | ?^' + str(m) + ' 0:?^' + str(n-1) + ']*')
        padb = FST('[[0:?^' + str(m-1) + ' - 0:□^' + str(m-1) + ' ] 0:□^' +
                   str(n) + '| 0:?^' + str(m-1) + ' ?^' + str(n) + ']*')
        extenda = self.compose(pada).lower()
        extendb = other.compose(padb).lower()
        flt = FST('~[?^' + str(m+n-1) + '* [□^' + str(m) + ' ?^' + str(n-1) + ' [?^' + str(m-1) + ' □^' + str(n) + ' |[?^' + str(m-1) + ' - □^' + str(m-1) + ' ] □ [?^' + str(n-1) + ' - □^' +
                  str(n-1) + ' ]] | ?^' + str(m-1) + ' □^' + str(n) + ' [□^' + str(m) + ' ?^' + str(n-1) + ' |[?^' + str(m-1) + ' - □^' + str(m-1) + ' ] □ [?^' + str(n-1) + ' - □^' + str(n-1) + ' ]]] ?*]')
    ...
    def _mtwords(self):
        ...
        toksym = '\x07'
        foma_apply_set_space_symbol(
            c_void_p(applyerhandle), c_char_p(self.encode(toksym)))
        output = applyf(c_void_p(applyerhandle))
        while True:
            if output is None:
                foma_apply_clear(c_void_p(applyerhandle))
