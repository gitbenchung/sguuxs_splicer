...
class Parser:
    ...
    load_input = path to a foma file or json configuration file
                default: 'fst/full_sgx.json'
    """

    def __init__(self, load_input: str | dict = FULL_SGX) -> None:
        self.reload(load_input)

    def reload(self, load_input: str | dict) -> None:
...
""" def paradigmize(self, form: string) -> list:
        
        Input a foma root +N; return list of possible concatenated forms.
        Convert to df as readable table.
...
"""
        ...
        """ Not sure exactly where to start if I am being honest. I know I need to run the results (lemmatized string) through foma
            based on sgx_inflection and just this file for now.
...
"""
        ...
        df = pd.DataFrame(list(paradigm_results), columns=["Surface Form", "Morphemes"])
            df.insert(1, "Person & Number", ["Surface Form", "Morphemes"])
            
            # Use lots of If-Else conditions to insert layman text "First person, singular 'my'" in Person & Number column?

            title_row = ["Possessive Word-Stem for:", results] #not sure I can draw back the older results from lemmatize
            
                df.columns = pd.MultiIndex.from_tuples(zip(title_row, df.columns))
